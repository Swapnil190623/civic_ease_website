...
const DashboardCitizen = () => {
  ...
  const myIssues = issues.filter((issue) => issue.reported_by === currentUser.id);
  const displayIssues = activeTab === "myIssues" ? myIssues : issues;
  const filteredIssues =
    filterCategory === "all"
      ? displayIssues
      : displayIssues.filter((issue) => issue.category === filterCategory);
  ...
  const getPriorityColor = (priority) => {
    if (priority === "high") return "text-red-600 bg-red-50";
    if (priority === "medium") return "text-emerald-600 bg-emerald-50";
  ...
  const IssueCard = ({ issue }) => {
    const isVoted = votedIssues.includes(issue.id);
    const isMyIssue = issue.reported_by === currentUser.id;
    ...
    return (
      <div className="bg-white rounded-2xl border border-gray-200 hover:shadow-lg transition-all overflow-hidden">
        <div className="p-6 pb-4">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-start space-x-3">
              <div className="w-12 h-12 bg-linear-to-br from-teal-500 to-emerald-600 rounded-full flex items-center justify-center">
                <span className="text-white font-bold text-lg">
                  {issue.anonymous ? "?" : (issue.reportedBy || "U")[0]}
                </span>
              </div>
              <div>
                <p className="font-bold text-gray-900">
                  {issue.anonymous ? "Anonymous" : issue.reportedBy || "Citizen"}
                </p>
        ...
        <div className="border-t border-gray-100 px-6 py-3 flex justify-between items-center">
          <div className="flex gap-2">
            ...
            >
              ...
              <span>{issue.issue_votes || 0}</span>
            </button>
            <button className="flex items-center gap-2 px-4 py-2 rounded-lg font-medium bg-gray-100 hover:bg-gray-200">
              <MessageCircle className="w-4 h-4" />
              <span>Comments</span>
            </button>
          </div>
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-emerald-50 to-teal-100">
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 bg-linear-to-br from-teal-500 to-emerald-600 rounded-xl flex items-center justify-center text-white font-bold shadow">
              <Zap className="w-6 h-6" />
            </div>
            <span className="text-2xl font-bold text-gray-800">CivicEase</span>
          </div>
          <div className="flex items-center gap-4">
            <button className="relative p-2.5 hover:bg-emerald-100 rounded-lg transition">
            ...
            <LogOut className="w-5 h-5 text-gray-600 hover:text-teal-500 cursor-pointer" />
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 grid lg:grid-cols-12 gap-8">
        {/* Left Sidebar */}
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-white rounded-2xl p-6 shadow border border-gray-200 sticky top-24">
            <div className="text-center mb-6">
              <div className="w-20 h-20 mx-auto bg-teal-400 rounded-full flex items-center justify-center text-white text-2xl font-bold mb-3">
                {currentUser.name[0]}
              </div>
              <h3 className="font-bold text-gray-900 text-lg">
                {currentUser.name}
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between p-3 bg-teal-50 rounded-xl">
            ...
            <button onClick={() => window.location.href = "/report"} className="w-full mt-6 px-6 py-3 bg-linear-to-r from-teal-500 to-emerald-600 text-white font-semibold rounded-xl shadow hover:scale-105 transition-all duration-200 flex items-center justify-center gap-2">
              <Plus className="w-5 h-5" />
              <span className="cursor-pointer">Report New Issue</span>
            </button>
          </div>
        </div>
        ...
        <div className="lg:col-span-6 space-y-6">
          {/* Tabs */}
          <div className="bg-white rounded-2xl p-2 shadow border border-gray-200 flex">
            <button
              onClick={() => setActiveTab("feed")}
          ...
          {/* Filter + Search */}
          <div className="bg-white rounded-2xl p-4 shadow border border-gray-200 flex justify-between items-center">
            <div className="flex gap-2 items-center overflow-x-auto scrollbar-hide group pt-2">
              <Filter className="w-5 h-5 text-gray-400 shrink-0" />
              {["all", "Roads & Infrastructure", "Sanitation & Cleanliness", "Electricity & Power"].map(
                (cat) => (
                  <button
                    key={cat}
                    onClick={() => setFilterCategory(cat)}
                    className={`px-4 py-2 mb-2 rounded-lg font-medium whitespace-nowrap ${
                      filterCategory === cat
                        ? "bg-emerald-500 text-white"
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search..."
                className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none"
              />
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center bg-white p-10 rounded-2xl shadow border border-gray-200">
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow border border-gray-200">
            ...
            <div className="space-y-3">
              {issues
                .sort((a, b) => (b.issue_votes || 0) - (a.issue_votes || 0))
                .slice(0, 5)
                .map((issue, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-2 p-2 rounded-lg hover:bg-orange-50 cursor-pointer"
                  >
          ...
          <div className="p-6 bg-white rounded-2xl shadow-xl"> 
            ...
            <div className="space-y-2">
              <p className="flex justify-between">
                <span className="text-teal-500 font-semibold">Active Citizens</span> <strong>200+</strong>
              </p>
              <p className="flex justify-between">
                <span className="text-teal-500 font-semibold">Issues Resolved</span> <strong>500+</strong>
              </p>
              <p className="flex justify-between">
                <span className="text-teal-500 font-semibold">Avg Resolution</span> <strong>3.5 days</strong>
              </p>
            </div>
          </div>
        </div>
