...
import { 
  Shield, Bell, X, FileText, Clock, TrendingUp, CheckCircle, AlertCircle, Users, MapPin, Filter, Search, Download, MoreVertical, Calendar, BarChart3, Eye, Edit, Trash2 
} from 'lucide-react';
...
function DashboardAdmin() {
  ...
  useEffect(() => {
    ...
    if (filter)
      data = data.filter((i) => i.status?.toLowerCase() === filter.toLowerCase());
    setFiltered(data);
  }, [search, filter, issues]);

  // Pagination
  const totalPages = Math.ceil(filtered.length / issuesPerPage);
  const start = (currentPage - 1) * issuesPerPage;
  const visibleIssues = filtered.slice(start, start + issuesPerPage);

  const handleCloseModal = () => {
    setSelectedIssue(null); // hides the modal
  };
    ...
    <div className="flex bg-teal-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatsCard
          ...
          subtitle="All Issues"
          extra={<><TrendingUp className="w-3 h-3 mr-1" />+12% from last week</>}
        />
        <StatsCard
          title="Pending"
          value={stats.pending}
          ...
          extra={<><AlertCircle className="w-3 h-3 mr-1" />Requires attention</>}
        />
        <StatsCard
          title="In Progress"
        ...
        <StatsCard
          ...
          subtitle="Resolved"
          extra={<><CheckCircle className="w-3 h-3 mr-1" />Great work!</>}
        />
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-8">
        <StatsCard
          ...
          icon={AlertCircle}
          bgColor="bg-gradient-to-br from-purple-500 to-blue-300 text-white"
        ...
        <StatsCard
          ...
          icon={Users}
          bgColor="bg-gradient-to-br from-blue-500 to-cyan-300 text-white"
        ...
        <StatsCard
          ...
          icon={BarChart3}
          bgColor="bg-gradient-to-br from-green-500 to-yellow-200 text-white"
        ...
        <div className="my-10"> 
          {visibleIssues.map((issue) => (
            <IssueCard key={issue.id} issue={issue} onView={setSelectedIssue} />
          ))}
        </div>
